...
The command opens the store with `readOnly: true` and never migrates. A missing path prints `no decisions` / `[]` and exits `0` without creating files. Zero-mutation promise = main-file bytes, mode, `user_version`, and all directory entries other than `<db>-wal` / `<db>-shm` (a WAL-mode DB may require those sidecars for any reader; audit may create or leave them). If the directory is unwritable and SQLite cannot open (`CANTOPEN`), audit prints `qlb audit: cannot open store read-only (<reason>)`, exits `1`, prints no stack, and creates no new entries. `--limit` must be a positive safe integer; an invalid value exits `1` with a single stderr line.
